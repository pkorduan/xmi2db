	...
	*/
	...
	function buildSchema() {
		...
		if ($schemaBool[0]=='t') {
		  ...
		  //echo $sql_dump;
		  $sql_dump2 = str_replace('_uml_schema_name_', $_REQUEST['schema'], $sql_dump);
		  //echo $sql_dump2;
  ...
  function getQueries() {
	...
	if(!isset($this->basePackage) or $this->basePackage=='') {
      ...
      foreach ($xmi->{$this->root_element}->children($namespaces["UML"])->Model->{'Namespace.ownedElement'}->Package as $package_top) {
        ...
        if (isset($this->basePackage) and $package_top->attributes()->name == $this->basePackage and $this->basePackage!=='') $this->iterateModel($package_top);//$root_package = $package_top;
        else if(!isset($this->basePackage) or $this->basePackage=='') $this->iterateModel($package_top);
      }
    }
